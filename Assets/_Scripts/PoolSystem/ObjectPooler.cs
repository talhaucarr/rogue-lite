...
namespace Utilities
{
    public sealed class ObjectPooler : AutoSingleton<ObjectPooler>
    {
        ...
        public GameObject Spawn(string poolName, Vector3 position, Quaternion rotation = new(),
            Transform parentTransform = null)
        {
            ...
            foreach (GameObject poolObj in allPools[pool].PooledObjects.Where(poolObj => !poolObj.activeSelf))
            {
                // Set active:
        ...
        public List<Pool> extraPools = new();
        ...
        {
            poolProfile =
                Resources.Load<PoolProfile>(
                    "ScriptableObjects/Pools/PoolProfiles/" + SceneManager.GetActiveScene().name);
            if (!poolProfile)
            {
                Debug.Log("Scene doesnt have a PoolProfile Using FallbackPoolProfile");
                poolProfile =
                    Resources.Load<PoolProfile>("ScriptableObjects/Pools/PoolProfiles/" + "FallbackPoolProfile");
            }
        }

        private void CollectPools()
        {
            List<PoolPack> poolPacks = poolProfile.poolPacks;
            foreach (PoolPack poolpack in poolPacks)
            {
                List<Pool> pools = poolpack.pools;
                foreach (Pool pool in pools) allPools.Add(Pool.CopyOf(pool));
            }

            foreach (Pool pool in extraPools) allPools.Add(Pool.CopyOf(pool));
        }
        ...
        private void InstansiatePoolObjects(List<Pool> pools)
        {
            ...
            {
                var poolParent = new GameObject { name = pool.Prefab.name };
                poolParent.transform.parent = transform;
                pool.StartingParent = poolParent.transform;

                for (var i = 0; i < pool.StartingQuantity; i++)
                {
                    GameObject obj = Instantiate(pool.Prefab, Vector3.zero, Quaternion.identity, poolParent.transform);
